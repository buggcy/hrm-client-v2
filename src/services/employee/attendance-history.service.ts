...
import { attendanceHistoryApiResponseSchema } from '@/libs/validations/attendance-history';
...
export const searchAttedanceHistoryList = async ({
  ...
  month: number;
  year: number;
}): Promise<AxiosResponse<AttendanceHistoryApiResponse>> => {
...
export const getAttendanceHistoryStats = async ({
  ...
  month: number;
  year: number;
}): Promise<AxiosResponse<AttendanceApiResponse>> => {
  const res = await baseAPI.get(
